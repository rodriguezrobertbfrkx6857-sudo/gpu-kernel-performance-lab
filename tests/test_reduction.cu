...
int main() {
    ...
    const auto run_case = [&](std::size_t n) {
        ...
        for (std::size_t i = 0; i < n; ++i) {
            input[i] = std::sin(static_cast<float>(i) * 0.001f);
        }
        ...
        for (const auto& variant : variants) {
            ...
            GPU_LAB_CUDA_CHECK(cudaMemcpy(device_output, &sentinel, sizeof(float),
                                          cudaMemcpyHostToDevice));
            variant.launch(device_input, device_output, n, nullptr);
            GPU_LAB_CUDA_CHECK(cudaDeviceSynchronize());
            float actual = 0.0f;
            GPU_LAB_CUDA_CHECK(cudaMemcpy(&actual, device_output, sizeof(float),
    ...
    run_case(31);                 // below one 256-thread block
    run_case(256);                // exact block multiple
    run_case(1003);               // irregular length
    run_case((1u << 20) + 17u);   // larger irregular input
    gpu_lab::test::pass("reduction variants, reset, and irregular lengths");
    return 0;
}
